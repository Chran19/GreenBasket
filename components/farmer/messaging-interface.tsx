...
import { Send, Search, MessageCircle, Star, Clock } from "lucide-react";
import { farmerAPI, usersAPI } from "@/lib/api";
...
export function MessagingInterface() {
  ...
  const fetchUserInfo = async (userId: string) => {
    ...
    return { id: userId, name: "Buyer" };
  };

  // Load conversations list
  const loadConversations = async () => {
    try {
      const res: any = await farmerAPI.getMessages();
        ...
        if (found) {
          ...
          try {
            ...
            const placeholder: Conversation = {
              id: conversationParam,
              partner: { id: conversationParam, name: "Buyer" },
              lastMessage: {
                content: "",
                created_at: new Date().toISOString(),
                isFromMe: false,
  ...
  const loadMessages = async (conversationId: string) => {
    ...
    try {
      const res: any = await farmerAPI.getConversation(conversationId, 1, 100);
      ...
      setConversations((prev) =>
        prev.map((conv) => {
          if (conv.id === conversationId) {
            ...
            if (partnerName && updated.partner.name === "Buyer") {
  ...
  //   }
  // }, [])
  // Poll for new messages safely (only one interval active)
  useEffect(() => {
    loadConversations();
  ...
  const sendMessage = async () => {
    ...
    try {
      await farmerAPI.sendMessage(selectedConversation.id, messageText);
